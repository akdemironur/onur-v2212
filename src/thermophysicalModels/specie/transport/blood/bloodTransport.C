...
Foam::bloodTransport<Thermo>::bloodTransport(const dictionary& dict)
:
    Thermo(dict),
...
template<class Thermo>
void Foam::bloodTransport<Thermo>::bloodTransport::write(Ostream& os) const
{
...
template<class Thermo>
Foam::Ostream& Foam::operator<<(Ostream& os, const bloodTransport<Thermo>& ct)
{
    ct.write(os);
    return os;
}
